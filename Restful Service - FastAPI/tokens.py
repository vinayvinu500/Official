...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="login") # fetch the token from the login route


async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> GET_Token:
